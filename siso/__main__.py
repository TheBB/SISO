...
from .api import Endianness, Dimensionality, Staggering, Source, ReaderSettings
from .field import Field, FieldType
from .multisource import MultiSource
from .reader import find_reader, FindReaderSettings
...
@click.option('--volumetric', 'dimensionality', flag_value=Dimensionality.Volumetric, default=True, type=click.UNPROCESSED)
...
def main(
    ...
    source = find_source(inpath, FindReaderSettings(
        endianness=in_endianness,
    ))
    if not source:
        sys.exit(2)
    source.configure(ReaderSettings(
        endianness=in_endianness,
        dimensionality=dimensionality,
        staggering=staggering,
    ))
